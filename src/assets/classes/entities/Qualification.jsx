...
export default class Qualification {
    ...
    create() {
        return new Promise((res, rej) => {
            ...
            axios({
                method: 'post',
                url: configApi.getCvPersistenceUrl()+"qualifications",
    ...
    list(){
        return new Promise((res, rej) => {
            ...
            axios({
                method: 'get',
                url: configApi.getCvPersistenceUrl()+"qualifications/"+this.idCurriculum,
            })
                .then((data) => {
                    res(data)
                })
    ...
    delete() {
        return new Promise((res, rej) => {
            ...
            axios({
                method: 'delete',
                url: configApi.getCvPersistenceUrl()+"qualifications/"+this.idCurriculum+"/"+this.idQualification
            })
                .then((data) => {
                    if (data.data.errMessage!=undefined){
                        rej(data)
    ...
    getFirst(){
        return new Promise((res, rej) => {
            ...
            this.list()
            .then((response) => {
                let firstQualification=response.data.Items[0]
                if (firstQualification){
                    res(firstQualification)
                }else{
                    rej(false)
                }
            })
